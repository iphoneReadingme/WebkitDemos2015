...
NSString * const NBCoreTextFontDescriptorException = @"NBCoreTextFontDescriptorException";
...
@implementation NBFontDescriptor
{
...
+ (void)_createDictionaryOfAllAvailableFontOverrideNamesWithCompletion:(void(^)(NSDictionary *dictionary))completion
{
	...
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
		...
		NBFontCollection *allFonts = [NBFontCollection availableFontsCollection];
		...
		for (NBFontDescriptor *oneFontDescriptor in sortedFonts)
		{
...
+ (void)setFallbackFontFamily:(NSString *)fontFamily
{
	if (!fontFamily)
	{
		[NSException raise:NBCoreTextFontDescriptorException format:@"Fallback Font Family cannot be nil"];
	}
	...
	if (!isValid)
	{
		[NSException raise:NBCoreTextFontDescriptorException format:@"Fallback Font Family '%@' not registered on the system", fontFamily];
	}
...
- (id)initWithCTFontDescriptor:(CTFontDescriptorRef)ctFontDescriptor
{
	...
	if (self)
	{
		...
		CFDictionaryRef traitsDict = (CFDictionaryRef)CTFontDescriptorCopyAttribute(ctFontDescriptor, kCTFontTraitsAttribute);
		...
		CFStringRef familyName = (CFStringRef)CTFontDescriptorCopyAttribute(ctFontDescriptor, kCTFontFamilyNameAttribute);
...
- (id)initWithCTFont:(CTFontRef)ctFont
{
   NSParameterAssert(ctFont);
	...
	if (self)
	{
		...
		CFDictionaryRef traitsDict = (CFDictionaryRef)CTFontDescriptorCopyAttribute(fd, kCTFontTraitsAttribute);
		CTFontSymbolicTraits traitsValue = [[(__bridge NSDictionary *)traitsDict objectForKey:(id)kCTFontSymbolicTrait] unsignedIntValue];
		CFRelease(traitsDict);
		CFRelease(fd);
...
- (NSDictionary *)fontAttributes
{
	...
	if (!self.boldTrait && _needsChineseFontCascadeFix)
	{
		...
		[tmpDict setObject:[NSArray arrayWithObject:(id) desc] forKey:(id)kCTFontCascadeListAttribute];
	}
...
{
	if ([NBFontDescriptor smallCapsFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait])
	{
		return YES;
	}
...
{
	NSDictionary *traits = (NSDictionary *)CTFontCopyTraits(font);
...
- (CTFontRef)_findOrMakeMatchingFont
{
	...
	CTFontRef cachedFont = (CTFontRef)[_fontCache objectForKey:cacheKey];
	...
	if (_fontFamily && !_fontName) // forced font name takes precedence
	{
		if (_smallCapsFeature)
		{
			overrideName = [NBFontDescriptor smallCapsFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait];
		}
		else
		{
			overrideName = [NBFontDescriptor overrideFontNameforFontFamily:_fontFamily bold:self.boldTrait italic:self.italicTrait];
		}
	}
...
- (BOOL)isEqual:(id)object
{
	...
	if (![object isKindOfClass:[NBFontDescriptor class]])
	{
		return NO;
	}
	
	NBFontDescriptor *otherFontDescriptor = object;
...
{
	NBFontDescriptor *newDesc = [[NBFontDescriptor allocWithZone:zone] initWithFontAttributes:[self fontAttributes]];
