...
+ (void)initialize
{
	...
	if (self != [NBFontDescriptor class])
	{
		return;
	}
	...
	for (NSDictionary *oneOverride in fileArray)
	{
		...
		if (smallcaps)
		{
			[NBFontDescriptor setSmallCapsFontName:overrideFontName forFontFamily:fontFamily bold:bold italic:italic];
		}
		else
		{
			[NBFontDescriptor setOverrideFontName:overrideFontName forFontFamily:fontFamily bold:bold italic:italic];
		}
	}
