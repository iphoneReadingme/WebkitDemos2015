...
- (BOOL)addContactName:(NSString*)name phoneNum:(NSString*)num withLabel:(NSString*)label{
	...
	ABMultiValueIdentifier *outIdentifier = nil;
	ABMultiValueAddValueAndLabel(multi, (__bridge CFTypeRef)num, (__bridge CFStringRef)label, outIdentifier);
...
-(NSMutableDictionary*)getPersonInfo{
    ...
    for (NSDictionary*dic in self.dataArrayDic) {
        ...
        NSLog(@"%@",dic);
		NSString *strFirLetter = nil;//[NSString stringWithFormat:@"%c",pinyinFirstLetter([str characterAtIndex:0])];
